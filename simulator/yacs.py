...
class Simulador:
    ...
    def asignar_proceso_a_core(self, proceso):
        for core in self.cores:
            if not core.ocupado:
                core.asignar_proceso(proceso, self.calcular_tiempo_servicio(proceso))
                return
    ...
    def simular(self, procesos):
        ...
        throughput = sum([core.tiempo_utilizacion for core in self.cores]) / len(procesos)
        utilizacion_cores = [(core.ID, core.tiempo_utilizacion) for core in self.cores]
...
simulador = Simulador(num_cores=4, tamaño_L1=5, tamaño_L2=10, tiempo_L1=4, tiempo_L2=10, tiempo_RAM=200)
